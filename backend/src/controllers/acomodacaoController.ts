...
export const listarAcomodacoes = async (_req: Request, res: Response) => {
  try { res.json(await service.listarAcomodacoes()) }
  catch (e: any) { res.status(500).json({ erro: e.message }) }
}

export const buscarAcomodacao = async (req: Request, res: Response) => {
  try { res.json(await service.buscarAcomodacao(Number(req.params.id))) }
  catch (e: any) { res.status(404).json({ erro: e.message }) }
}

export const criarAcomodacao = async (req: Request, res: Response) => {
  try {
    const { nome } = req.body as { nome: NomeAcomodacao }
    res.status(201).json(await service.criarAcomodacao(nome))
